...
def mint_with_sso_protocol(
    *,
    ...
    body_snip = ""
    try:
        body_snip = (r.text or "")[:2000]
    except Exception:
        pass

    # Explicit error marker in final URL (e.g. ?error=rate_limited) — fail fast.
    if "error=" in verify_url.split("?", 1)[-1]:
        raise ProtocolMintError(
    ...
    status = getattr(r, "status_code", 0)
    if "error=" in approve_url.split("?", 1)[-1]:
        raise ProtocolMintError(
    ...
    if "done" not in approve_url and "device/done" not in _url_path(approve_url):
        # Accept only strong success-page markers. NOTE: a bare "done" in the
        # body is NOT enough — Next.js SPAs contain "done" inside JS strings,
        # which caused silent approve failures (token poll → invalid_grant).
        try:
            text = r.text or ""
        except Exception:
            text = ""
        if "设备已授权" not in text and "device authorized" not in text.lower():
            raise ProtocolMintError(
                f"device/approve failed status={status} url={approve_url[:160]}"
            )
