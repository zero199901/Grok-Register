...
def mint_and_export(
    *,
    ...
    if probe:
        ...
        free_model = pr.get("free_model") or ("grok-4.5" if pr.get("has_grok_45") else "")
        log(
            f"probe models: ok={pr.get('ok')} status={pr.get('status')} "
            f"free_model={free_model or '-'} has_grok_45={pr.get('has_grok_45')} "
            f"ids={pr.get('model_ids')} "
            f"error={str(pr.get('error') or '')[:200]}"
        )
        if not free_model:
            result["ok"] = False
            # Keep the historical error prefix: cpa_export.py ignores this
            # warning (file already written) when cpa_probe_required=false.
            result["error"] = f"token ok but grok-4.5 not listed (ids={pr.get('model_ids')})"
        if probe_chat and free_model:
