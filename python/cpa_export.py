...
def export_cpa_xai_for_account(
    ...
    *,
    ...
    # failure log under register dir (single-line: browser errors may span lines)
    if not result.get("ok"):
        fail_path = out_dir / "cpa_auth_failed.txt"
        err = str(result.get("error") or "unknown")
        err = re.sub(r"\s+", " ", err).strip()[:500]
        proto_err = str(result.get("protocol_error") or "")
        proto_err = re.sub(r"\s+", " ", proto_err).strip()[:300]
        line = f"{email}----{err}"
        if proto_err and proto_err not in err:
            line += f" (protocol: {proto_err})"
