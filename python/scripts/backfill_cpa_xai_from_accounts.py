...
def main() -> int:
    ...
    # Auto-fallback: default ledger accounts_cli.txt may not exist (older runs
    # wrote timestamped accounts_YYYYMMDD_HHMMSS.txt ledgers).
    if not Path(args.accounts).is_file():
        import glob as _glob

        cands = sorted(
            _glob.glob(str(_ROOT / "accounts_*.txt")), key=lambda p: Path(p).stat().st_mtime
        )
        if cands:
            print(f"ledger {args.accounts} missing — using newest: {cands[-1]}", flush=True)
            args.accounts = cands[-1]
        else:
            print(f"error: ledger not found: {args.accounts}", flush=True)
            return 1
